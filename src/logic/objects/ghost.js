...
class Ghost {
    constructor({ position, indices, velocity, width, height, color = "red", promixityRadius = null }) {
        ...
        this.state = null;
        this.distanceLimit = this.width * gameConfig.MAP.DISTANCE_LIMIT;  // This is the min distance below which a ghost will be considered to have reached a target position
    ...
    draw(ctx, { showGhostProximityCircle = null }) {
        ...
        const legRadiusX = gameConfig.GHOST.LEGS.RADIUS_PERC * this.width;
        const legRadiusY = gameConfig.GHOST.LEGS.RADIUS_PERC * this.height;
        ...
        // Draw eyes
        const eyeRadius = gameConfig.GHOST.EYES.RADIUS_PERC * this.width;
        const eyeOffsetX = gameConfig.GHOST.EYES.OFFSET_PERC.X * this.width;
        const eyeOffsetY = gameConfig.GHOST.EYES.OFFSET_PERC.Y * this.height;
        ctx.beginPath();
        ...
        ctx.fillStyle = gameConfig.GHOST.EYES.COLOR;
        ctx.fill();

        // Draw pupils
        const pupilRadius = gameConfig.GHOST.EYES.PUPILS.RADIUS_PERC * eyeRadius;
        ctx.beginPath();
        ...
        ctx.fillStyle = gameConfig.GHOST.EYES.PUPILS.COLOR;
        ctx.fill();

        // Draw legs (three ellipses at the bottom)
        ctx.beginPath();
        for (let i = 0; i < gameConfig.GHOST.LEGS.COUNT; i++) {
